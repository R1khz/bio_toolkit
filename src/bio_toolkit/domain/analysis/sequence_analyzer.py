...
class SequenceAnalyzer:
    ...
    def analyze_record(self, record: SeqRecord) -> dict[str, Any]:
        ...
        result["analysis"]["warnings"] = build_analysis_warnings(
        ...
        for frame_seq, frame_name, offset in frames:
            ...
            while True:
                ...
                if len(protein) >= self.min_orf_aa:
                    coding_sequence = str(frame_seq[methionine_index * 3 : stop_index * 3]).upper()
                    orfs.append(
                        {
                            "frame": frame_name,
...
def _analyze_nucleotide(record: SeqRecord) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "ambiguous_content": round((ambiguous_count / informative_length) * 100, 2)
